...
User.linkSave = function(user, callback) {
  console.log('updating the new local User');
  var id = user.u_id;
  var email = user.email;
  var password = user.password;
  console.log(email, password, 'and', id);

  var sql = 'UPDATE users SET email =$1, password = $2 WHERE u_id=$3 RETURNING u_id, email, password';
  var data = [
    email,
    password,
    id
...
User.linkSaveFB = function(user, callback) {
  console.log('saving the new facebook User', user);
  var id = user.u_id;
...
User.linkSaveGoogle = function(user, callback) {
...
User.findOneGoogle = function(token, callback) {

  var isNotAvailable = false; //we are assuming the email is taking

  console.log(token + ' is in the findOne function test');
  //check if there is a user available for this email;
  var sql = 'SELECT * FROM users WHERE google_token = $1';
  var data = [
    token
  ];
